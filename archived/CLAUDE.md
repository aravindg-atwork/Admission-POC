# Working notes for the archived POC backend (`archived/backend/`)

**Moved here from the repo root on 2026-08-21.** `platform/` is now the
primary system — it's what's live on the production server's port 80
(`http://159.69.210.30/`). This backend is kept fully working and fully
documented, not deleted: it's still reachable in production at
`http://159.69.210.30/legacy/` (proxied) and directly on the host at
port `5050`. See `../GUIDE.md` at the repo root for the current
top-level orientation, and `../platform/CLAUDE.md` for the primary
system's own working notes.

**Every repo-root-relative path below (`backend/`, `tools/`,
`run_backend.py`, `data/`, `.env`) needs one translation**: this whole
file predates the move and was kept as-is rather than rewritten line by
line. `backend/` and `tools/` are now `archived/backend/` and
`archived/tools/` (i.e. exactly where this file itself lives); `data/`
and the real `.env` were **not** moved and are still at the true repo
root (`platform/`'s own ingestion scripts read from `data/` too, so it
stays shared). Run commands from the repo root exactly as written below,
just remember `run_backend.py` now means `archived/run_backend.py` and
`tools/whatever.py` now means `archived/tools/whatever.py`.
