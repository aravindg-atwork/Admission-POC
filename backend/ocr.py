...
def available():
    return bool(config.OCR_ENABLED and config.MISTRAL_API_KEY)
...
def extract_pages(pdf_path):
    ...
    try:
        ...
        req = urllib.request.Request(
            config.MISTRAL_URL.rstrip("/") + "/ocr", data=payload, method="POST",
            headers={
                "Content-Type": "application/json",
                "Authorization": f"Bearer {config.MISTRAL_API_KEY}",
                "User-Agent": "AdmissionAssistant/1.0",
            })
        with urllib.request.urlopen(req, timeout=config.OCR_TIMEOUT) as resp:
